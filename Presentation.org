...
3. Increase the number of firms to 150, the exit threshold =sMin= to 0.001, and the number of simulation steps.
...
How can we investigate how parameters and initial conditions *jointly* affect a variable?
...
*Good news:* LSD (+R) can handle them on a NNF approach
...
Which parameters do you expect to be the most relevant for *HHI*? And for *aAvg*?
...
- In the Rscript, uncomment =varName   <- "aAvg"= and source the file again
